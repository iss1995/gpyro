...
extrap_in = [(copy(m), copy(g), copy(f), file_id, copy(validation_experiment), copy(likelihoods), copy(models), copy(GP_weights_normalizers), copy(prc), copy(delay_model), save_plots_, RESULTS_FOLDER, starting_point, steps) for (file_id,validation_experiment) in zip(files_to_evaluate,validation_experiments) ]
